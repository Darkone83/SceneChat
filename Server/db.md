...
If you ran the `scenechat_v13.sql` baseline, a set of default rooms (`general`, `off-topic`, `hardware`, `softmod`, `dev`, `showcase`, `voice1`, `voice2`) is already created — no further seeding is needed.

For a manual setup, at least one room is needed before any client can join. Create rooms from the admin panel, or insert directly:
...
| 1.3 | 2026-05-31 | DMs — `type` extended on `rooms`, new `room_participants` table |
| 1.3 | 2026-05-31 | Mailbox — new `mailbox` table |

---

*See ROADMAP.md for the v1.4 (video chat) feature plan.*