...
void InitInput()
{
    /* Register all device types including voice.
       Voice is a sub-device of the controller port so gamepad
       must also be registered or XVoiceCreateMediaObject crashes. */
    {
        XDEVICE_PREALLOC_TYPE types[5];
        ...
        types[2].DeviceType = XDEVICE_TYPE_VOICE_MICROPHONE;
        types[2].dwPreallocCount = 4;
        types[3].DeviceType = XDEVICE_TYPE_VOICE_HEADPHONE;
        types[3].dwPreallocCount = 4;
        types[4].DeviceType = XDEVICE_TYPE_DEBUG_KEYBOARD; /* enable USB keyboard enumeration */
        types[4].dwPreallocCount = 2;
        XInitDevices(5, types);
    }
